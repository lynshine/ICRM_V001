...
<script type="text/javascript">
	...
	$(function(){
		...
		$.get(url  , function(result){ // result -- 集合<Customer>
			...
			$(result).each(function(i , n){
				...
				$("#visit_cust_id").append("<option value='"+n.cust_id+"'>"+n.cust_name+"</option>")
			});
	...
	function changeCustomer() {
		var customerId = $("#visit_cust_id").val()
		$.post(
			"${pageContext.request.contextPath}/linkMan_findByCustID.action",
			{"custId":customerId},
			function(result){
				$("#visit_lkm_id").html("<option value=''>--请选择--</option>")
				$(result).each(function(i,n){
					$("#visit_lkm_id").append("<option value='"+n.lkm_id+"'>"+n.lkm_name+"</option>")
				})
			},
			"json"
		)
					...
					<td vAlign=top width="100%" bgcolor="#ffffff">
						...
						<table cellspacing="0" cellpadding="5" border="0">
							<tr>
								<td>客户名称：</td>
								<td>
									<select onchange="changeCustomer()" name="visit_cust_id.cust_id" id="visit_cust_id" class="textbox" style="width: 180px; height: 21px;">
							...
							<tr>
								<td>被拜访人：</td>
								<td>
									<select name="visit_lkm_id.lkm_id" id="visit_lkm_id" class="textbox" style="width: 180px; height: 21px;">
											<option value="">--请选择--</option>
									</select>
								</td>
								<td>拜访地点：</td>
